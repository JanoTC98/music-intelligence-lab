"""Evaluate the final multiclass model once on the frozen test split (§24.4).
...
once, after the model is selected (§17.4, §17.5). Without the flag the script
refuses to run.
"""
...
def main() -> None:
    ...
    if not args.use_test:
        raise SystemExit(
            ...
            "one-time final evaluation after model selection (§17.4)."
        )
