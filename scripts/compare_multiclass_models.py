"""Compare multiclass classifiers C0..C3 on validation.

Usage:
    uv run python scripts/compare_multiclass_models.py
    uv run python scripts/compare_multiclass_models.py --models C0 C1

Trains each requested model with the frozen grouped split, reports
metrics and the exploratory evaluation on multi-genre validation rows in
