...
- Evaluación offline (sección 14.11) en `reports/metrics/track_recommender_evaluation.json/.csv` (muestra de 200 semillas: 0 autorrecomendaciones, 0 duplicados, 100 % cumplimiento de filtros).
- Variante experimental de **afinidad de género** (sección 30) como toggle desactivado por defecto en la app (botón "Priorizar canciones del mismo género de la semilla"); reordena el pool acústico recuperado para que los candidatos que comparten género con la semilla salgan primero, sin añadir candidatos fuera del pool.
- Análisis de relevancia (sección 30, `scripts/analyze_recommender_relevance.py`, 2000 semillas) en `reports/experiments/recommender_relevance_analysis.json` — evaluado con las métricas oficiales (sección 14.11):
...
- Presets editables en `configs/presets.yaml` (7 presets con valores y pesos 0–3, sección 32.6) sin tocar Python.
- Distancia euclídea ponderada sobre variables escaladas (sección 15.6): el peso 0 ignora la variable y se rechazan los perfiles con todos los pesos en cero.
- Modo manual con pesos por variable (0–3) y modo preset.
- Detección de perfiles fuera de distribución (sección 15.8) por distancia al centroide del catálogo (percentiles p95/p99 precomputados).
- Diversidad MMR opcional (sección 15.9) con `lambda = 0,85`; el ranking puro es el predeterminado.
...
- Dataset multilabel con una fila por `recording_group_id` y matriz binaria `[n, 114]` (sección 16.2).
- Características primarias sección 16.3 (`log_duration`, `key_sin/cos`, `mode`, `time_signature` one-hot fija) sin columnas prohibidas de identidad.
- Split agrupado 70/15/15 congelado en `data/processed/splits.parquet` (hash `7cdb3f…b67`), intersecciones vacías y selección por menor desviación de prevalencia (sección 16.4).
- Experimentos A (excluye `audio_analysis_incomplete`) y B (imputación con medianas de train + indicador, sección 16.5); B no mejora a A (M1: samples F1 0.1395 vs 0.1386).
- Modelos M0 (frecuencia) y M1 (OneVsRest logistic, liblinear C=1.0) entrenados; M2 (ClassifierChain ×3), M3 (Extra Trees 400) y M4 (Random Forest 400) entrenados; M5/XGBoost deshabilitado (sección 16.6).
- Umbral global 0.10–0.90 optimizando samples F1 solo sobre validación (sección 16.8); la app muestra Top-5 con aviso de umbral no superado.
- Métricas sección 16.9 (macro/micro/samples F1, Hamming, precision@3, recall@5, hit@3/5, coverage, LRAP, AP por etiqueta) en `reports/experiments/classifier_multilabel_comparison.json`.
...
- `scripts/evaluate_final_model.py` evalúa el test congelado únicamente con `--use-test` (sección 16.4).
- Notebook `notebooks/06_multilabel_classifier.ipynb`.

### Módulo 7 · Clasificador multiclase de género dominante
- Dataset de grabaciones monoetiqueta (sección 17.1): 69.943 grupos; split agrupado congelado train 48.987 / validación 10.496 / test 10.460; 112/114 clases en train (conteos 47–717).
- Modelos C0 (clase frecuente), C1 (logística lbfgs C=1.0), C2 (Extra Trees) y C3 (Random Forest) con `max_depth=12` y `n_estimators=300` (sección 17.3).
...
- Métricas sección 17.4 sobre validación en `models/classifier/multiclass/<id_C1>/metrics_validation.json`.
- Evaluación exploratoria sección 17.5 sobre filas multigénero: validación Hit@1=0.195, Hit@3=0.393, Recall@5=0.284 (2.086 filas); test Hit@1=0.179, Hit@3=0.366, Recall@5=0.263 (2.123 filas).
...
- `scripts/evaluate_final_multiclass_model.py` evalúa el test congelado únicamente con `--use-test` (sección 17.4).
- Notebook `notebooks/07_multiclass_classifier.ipynb`.

### Módulo 8 · Aplicación Streamlit
- Aplicación multipágina (`streamlit_app.py` como router, sección 18.1) con 7 páginas: Inicio, Auditoría y catálogo, Recomendar por canción, Recomendar por preferencias, Laboratorio multietiqueta, Laboratorio de género dominante y Metodología y limitaciones.
- Carga de datos con `st.cache_data` y de modelos/escaladores/índices con `st.cache_resource` (sección 18.2).
- La app **no entrena** ante artefactos ausentes: muestra el mensaje "El artefacto requerido no existe. Ejecute el script de construcción correspondiente." (sección 18.4).
...
- El recomendador excluye la propia grabación y también otras grabaciones de la misma canción (mismo título + artistas en otro `recording_group_id`, sección 3.4) para no recomendar la misma obra como primera opción.
...
- Resultados descargables en CSV, explicaciones por característica y avisos de limitaciones; sin branding oficial de Spotify (sección 18.5).
- Tracking deshabilitado por defecto (`configs/app.yaml`); Módulo 9 opcional.

## Comparativa de modelos evaluados

### Tabla 0 · Recomendador por canción — R1–R4 (sección 14.4)

| Experimento | Escalador | Distancia | Similitud media | Latencia | Veredicto |
|---|---|---:|---:|---:|---|
| **R1** | Standard | coseno | 0,9794 | 14,6 ms | **Aprobado** (baseline sección 14.4) |
...
Evaluación sección 14.11 en producción (200 semillas, R1): 0 autorrecomendaciones · 0 duplicados de grupo · 100 % cumplimiento de filtros · similitud media 0,9729 · cobertura del catálogo 2,35 % · artistas únicos 0,94/lista · diversidad interna (std) 0,0059 · latencia p50 22,2 ms / p95 48,3 ms · estabilidad p50-Jaccard 0,919.

### Tabla 0b · Recomendador por preferencias (sección 15) — servicio desplegado

Sin comparación de modelos: es **no supervisado y determinista** (sección 15.10). Distancia euclídea ponderada sobre variables escaladas con pesos 0–3 (sección 15.6), 7 presets editables en `configs/presets.yaml` (Entrenamiento intenso, Fiesta, Concentración instrumental, Relajación, Alegre y bailable, Melancólico, Acústico), detección de perfiles fuera de distribución (percentiles p95/p99) y diversidad MMR opcional con `lambda = 0,85` — el ranking puro es el predeterminado.

### Tabla 1 · Clasificador multietiqueta (sección 16) — métricas de validación
...
### Tabla 2 · Clasificador multiclase (sección 17) — métricas de validación
