"""Multilabel genre lab page (AGENTS.md §16)."""
...
def render() -> None:
    ...
    if bool(recording_row.get("audio_analysis_incomplete", False)):
        messages.warn_note(
            "Esta grabación tiene análisis acústico incompleto (§4.1). En la "
    ...
    if st.button("Predecir géneros", type="primary", key="ml_run"):
        ...
        if prediction["below_threshold"]:
            messages.warn_note(
                ...
                "se muestra el top-1 con la advertencia correspondiente (§16.8)."
            )
